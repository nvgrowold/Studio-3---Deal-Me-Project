    ...
    <li className="relative p-6 bg-transparent flex flex-col justify-between shadow-xl hover:shadow-md rounded-md overflow-hidden transition-shadow duration-150 m-[10px]">
      ...
      <div className="mb-2">
        ...
        {order.timestamp && (
          <p className="text-lg  text-sky-800">
            ...
            <Moment format="DD/MM/YYYY">{order.timestamp.toDate()}</Moment>
          </p>
        )}
      </div>
      ...
      <p className=" text-sky-800">
        ...
        <span className="text-gray-500">
          $
          {order.userInfo && order.userInfo.totalPrice
            ? order.userInfo.totalPrice
                .toString()
                .replace(/\B(?=(\d{3})+(?!\d))/g, ",")
            : "N/A"}
        </span>
      ...
      {order.userInfo && (
        <div className="mt-4  text-sky-800">
          ...
          <div className="font-semibold">
            ...
            <p>
              Email:{" "}
              <span className="text-gray-500">{order.userInfo.email}</span>
            </p>
            <p>
              Delivery Address:{" "}
              <span className="text-gray-500">
